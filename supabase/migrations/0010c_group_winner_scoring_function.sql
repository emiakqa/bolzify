-- Bolzify — Gruppensieger-Scoring (Function + Trigger-Erweiterung)
-- Voraussetzung: 0010b wurde ausgeführt.
--
-- Wir ersetzen score_special_tips() komplett (CREATE OR REPLACE) — der neue
-- Code rechnet zusätzlich die Gruppensieger-Punkte und schreibt sie in die
-- in 0010b angelegten Spalten. Champion/Runner-up/HF/Top-Scorer-Logik ist
-- unverändert aus 0009b übernommen.
...
declare
  ...
  v_group_winners      jsonb;  -- { "A": 123, "B": 456, ... }
  v_tip                record;
  v_champion_pts       int;
  v_runner_up_pts      int;
  v_hf_hits            int;
...
begin
  ...
  -- Gruppensieger pro Gruppe ermitteln. Tabellen-Format: 3 P pro Sieg, 1 P
  -- pro Remis. Tie-Breaker: Punkte → Tordifferenz → erzielte Tore → team_id
  -- (deterministisch). NULL falls noch nicht alle 6 Gruppenspiele finished.
  with group_matches as (
    select
      m.stage,
      substring(m.stage from 'Group ([A-L])$') as grp,
      m.home_team_id, m.away_team_id, m.home_goals, m.away_goals
    from public.matches m
    where m.tournament = p_tournament
      and m.stage ~ '^Group [A-L]$'
      and m.status = 'finished'
      and m.home_goals is not null
      and m.away_goals is not null
  ),
  ...
  finished_groups as (
    -- Eine Gruppe gilt als „fertig", wenn 6 Spiele finished (4 Teams × 3
    -- Spiele / 2). Vorher kein Sieger ermittelbar.
  ...
  loop
    ...
    -- Gruppensieger-Treffer: Zähle group_winner_tips dieses Users, deren
    -- team_id mit v_group_winners[group_letter] übereinstimmt.
  ...
  -- Edge-Case: User hat NUR group_winner_tips (keine special_tips-Row).
  -- Dann nochmal über die durchlaufen, die der erste Loop verpasst hat.
  for v_tip in
...
-- Trigger erweitern: auch Gruppenspiele triggern Re-Scoring (sonst werden
-- Gruppensieger nie aktualisiert). Ersetzt die Function aus 0009c.
...
begin
  if new.status = 'finished'
     and (new.stage in ('Final', 'Semi-finals') or new.stage ~ '^Group [A-L]$')
     and (
