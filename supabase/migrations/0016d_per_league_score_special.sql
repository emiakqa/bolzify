-- Bolzify — Per-Liga-Tipps (Phase 3b: score_special_tips überschreiben)
-- Voraussetzung: 0016a/b/b2/c wurden ausgeführt.
--
-- Identisch zu 0012b_score_special_tips_v2.sql, nur mit league_id im
-- INSERT/CONFLICT. Champion/Runner-up/HF/Top-Scorer/Gruppensieger-Logik
-- ist league-agnostisch — bleibt 1:1.
...
begin
  select
    coalesce(m.winner_team_id,
      case when m.home_goals > m.away_goals then m.home_team_id
           when m.away_goals > m.home_goals then m.away_team_id
           else null end),
    case when m.winner_team_id is not null then
  ...
  where m.tournament = p_tournament and m.stage = 'Final' and m.status = 'finished'
    and m.home_goals is not null and m.away_goals is not null
  order by m.kickoff_at desc limit 1;

  select coalesce(array_agg(distinct t), array[]::int[]) into v_hf_teams
  from (
  ...
  order by p.tournament_goals desc, p.id asc limit 1;

  with group_matches as (
    select th.group_letter as grp, m.home_team_id, m.away_team_id, m.home_goals, m.away_goals
    from public.matches m
    join public.teams th on th.id = m.home_team_id
    join public.teams ta on ta.id = m.away_team_id
    where m.tournament = p_tournament
      and th.tournament = p_tournament and ta.tournament = p_tournament
      and th.group_letter is not null and ta.group_letter = th.group_letter
  ...
  per_team as (
    select grp, home_team_id as team_id,
           ...
           home_goals - away_goals as gd, home_goals as gf
    from group_matches
    union all
  ...
  totals as (
    ...
    from per_team where team_id is not null group by grp, team_id
  ),
  finished_groups as (
  ...
  winners as (
    ...
    from totals t join finished_groups f on f.grp = t.grp
    order by t.grp, t.pts desc, t.gd desc, t.gf desc, t.team_id asc
  )
  select coalesce(jsonb_object_agg(grp, team_id), '{}'::jsonb)
  into v_group_winners from winners;

  -- Pro (User, Liga) scoren — jeder User kann pro Liga eigene Sondertipps haben.
  for v_tip in
  ...
  loop
    v_champion_pts := case when v_champion_team_id is not null
                       and v_tip.champion_team_id = v_champion_team_id then 10 else 0 end;
    v_runner_up_pts := case when v_runner_up_team_id is not null
                        and v_tip.runner_up_team_id = v_runner_up_team_id then 5 else 0 end;
    v_hf_hits := (select count(distinct t)
                  from unnest(array[v_tip.champion_team_id, v_tip.runner_up_team_id,
                                    v_tip.semifinalist_a_team_id, v_tip.semifinalist_b_team_id]) as t
                  where t is not null and t = any(v_hf_teams));
    v_hf_pts := v_hf_hits * 5;
    v_top_scorer_pts := case when v_top_scorer_id is not null
                          and v_tip.top_scorer_player_id = v_top_scorer_id then 5 else 0 end;

    -- Gruppensieger-Treffer: nur tips DIESES Users in DIESER Liga zählen.
    select count(*)::int into v_gw_hits
    from public.group_winner_tips g
    where g.user_id = v_tip.user_id and g.tournament = p_tournament
      and g.league_id = v_tip.league_id
      and (v_group_winners ->> g.group_letter)::int = g.team_id;
    v_gw_pts := v_gw_hits * 3;

    insert into public.scored_special_tips (
      user_id, tournament, league_id,
      champion_points, runner_up_points,
      semifinalist_hits, semifinalist_points,
      top_scorer_points,
      group_winner_hits, group_winner_points
    ) values (
      v_tip.user_id, p_tournament, v_tip.league_id,
      v_champion_pts, v_runner_up_pts,
      v_hf_hits, v_hf_pts,
      v_top_scorer_pts,
      v_gw_hits, v_gw_pts
    )
    on conflict (user_id, tournament, league_id) do update
  ...
  -- Edge-Case: (User, Liga) hat NUR group_winner_tips, keine special_tips-Row.
  for v_tip in
    select distinct g.user_id, g.league_id
    from public.group_winner_tips g
  ...
  loop
    ...
    insert into public.scored_special_tips (
      user_id, tournament, league_id,
      group_winner_hits, group_winner_points
    ) values (v_tip.user_id, p_tournament, v_tip.league_id, v_gw_hits, v_gw_pts)
    on conflict (user_id, tournament, league_id) do update
